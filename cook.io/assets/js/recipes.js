...
const /**{NodeList} */ $filterToggglers = document.querySelectorAll("[data-filter-toggler]");
const /**{NodeElement} */ $overlay = document.querySelector("[data-overlay]");

addEventOnElements($filterToggglers, "click", function (){
...
// Parse query string safely and apply values to filter UI
const params = new URLSearchParams(window.location.search);
const /**{Array} */ queries = [];
for (const [k, v] of params.entries()) queries.push([k, v]);
...
// apply query values to UI (URLSearchParams returns decoded values)
for (const [key, value] of queries) {
    ...
    if (key === "q") {
        ...
        // find matching input by value (safer than building a selector with unescaped value)
        const $inputs = $filterBar.querySelectorAll('input');
        for (const $input of $inputs) {
            if ($input.value === value) { $input.checked = true; break; }
        }
    }
}
...
$gridList.innerHTML = $skeletonCard.repeat(25);
